package org.cloudbus.cloudsim.examples.GreedyScheduling;
...
import org.cloudbus.cloudsim.examples.GreedyScheduling.WorkFlowParser.Node;
	...
	protected void handleGreedyScheduling() {
		...
		if(firstInit) {
			...
			if(idleVmIdList.size()==0){
				return;
			}else if(idleVmIdList.size() > (taskList.size()/2)) {
				for(int vmId : id2VmMap.keySet()) {
					if(idleVmIdList.contains(vmId)) {
